...
## 2026-08-01：M1 首轮运行时与压力基线

本切片新增严格 C11 对照程序，以相同 SQL、Schema、4 MiB 每连接页缓存和编译选项测试系统 SQLite、CSGDB 明文与 CSGDB 加密公共接口。参考环境为 9 vCPU AMD EPYC 9V74 KVM、21 GiB 内存、ext4、GCC 13.3、Rust 1.97.1；100K 行数据库主文件约 19.3 MiB。

| 中位数指标 | SQLite | CSGDB 明文 | CSGDB 加密 |
| --- | ---: | ---: | ---: |
| 100K 行单事务插入 | 204.117 ms | 208.698 ms | 278.134 ms |
| 1K 次 FULL 自动提交 | 704.249 ms | 792.979 ms | 849.984 ms |
| 四读一写读取吞吐 | 780,773 ops/s | 358,224 ops/s | 202,031 ops/s |
| 四读一写更新吞吐 | 19,923 rows/s | 21,116 rows/s | 14,395 rows/s |
| 读取 p99 | 48.312 us | 82.393 us | 115.763 us |
| 写事务 p99 | 17.933 ms | 20.909 ms | 21.246 ms |
| 三轮压力错误 | 0 | 0 | 0 |

正式数据先预热，再旋转执行顺序并取三轮中位数；另对三种模式分别执行 60 秒持续压力，错误数仍全部为 0，总并发压力时间为 270 秒。明文单连接热路径已接近基线，但 C ABI 并发读取、加密缓存外随机访问和连接池总缓存预算需要优先优化。完整报告和适用边界见 [2026-08-01 运行时性能基线](performance-2026-08-01.md)。
