...
## 2026-08-01：M1 确定性 VFS 故障注入切片

本切片继续使用 Rust 1.97.1 工具链，最低 Rust 1.82 锁定检查保留为 CI 合入门槛。故障注入由默认关闭的 feature 隔离，正常构建不注册测试 VFS：

| 项目 | 结果 |
| --- | ---: |
| Rust 单元、宏、API 与集成测试 | 73 个通过 |
| 子进程故障 Worker 入口 | 1 个仅由父测试调用 |
| 确定性 VFS 恢复场景 | 4 个通过 |
| 全 workspace 全特性严格 Clippy | 通过 |
| 私有项 Rustdoc 严格警告检查 | 通过 |
...
四个恢复场景分别在 WAL 第一次目标写入中制造 7 字节部分写、在 `synchronous=FULL` 提交中制造 WAL 同步失败、在 FULL Checkpoint 中制造主文件写失败、在 TRUNCATE Checkpoint 中制造 WAL 截断失败。每条规则都断言实际命中一次，再通过默认 VFS 重开，验证已提交基线、未提交不可见性、Checkpoint 前已提交 WAL 数据和 `integrity_check`。部分写场景同时扫描加密主文件与 WAL，不允许出现已知明文标记。

该结果建立了可重复的软件 I/O 故障基线，不代表已完成真实设备掉电、文件系统谎报同步成功、扇区乱序或介质静默损坏验证。
