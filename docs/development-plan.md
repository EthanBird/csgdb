...
当前仍是开发版本。结构化查询 IR 的第一条端到端路径已经完成；尚未完成平台密钥库适配、底层部分写/同步失败注入、动态 AgentPlan、在线大表迁移和混合检索。

## 近期开发排期

按风险和依赖关系继续推进：

1. **存储故障注入**：在 VFS 边界稳定复现部分写、同步失败、截断和 WAL/主文件阶段故障，形成提交前后恢复矩阵；
2. **查询结果模型**：增加显式投影、主键游标分页、惰性解码和取消传播，同时保持每条路径有预算；
