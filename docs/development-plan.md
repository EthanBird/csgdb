...
当前仍是开发版本。结构化查询 IR 的第一条端到端路径和首批确定性存储故障矩阵已经完成；尚未完成真实设备掉电循环、文件系统谎报持久化、平台密钥库适配、动态 AgentPlan、在线大表迁移和混合检索。

## 近期开发排期

按风险和依赖关系继续推进：

1. **存储故障注入（首批完成）**：已经覆盖 WAL 部分写/同步、主文件 Checkpoint 写和 WAL 截断；下一轮补删除、目录同步、空间耗尽和目标设备真实掉电；
2. **查询结果模型（下一切片）**：增加显式投影、主键游标分页、惰性解码和取消传播，同时保持每条路径有预算；
3. **AgentPlan v1**：定义版本化、可序列化的受限计划，编译到已验证谓词 IR，强制 Namespace、Budget 和 Limit；
4. **计划解释与索引检查**：提供不含绑定值的 Explain 摘要、注册索引匹配检查和可观测执行统计；
5. **Agent 核心集合**：实现 Event、State、Memory、Fact、Edge 和 Provenance 的稳定 Schema、乐观并发与回放测试；
6. **设备基线**：补充 ARM64 交叉构建、固定数据规模基准和内存峰值门槛。
